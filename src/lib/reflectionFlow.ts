...
只返回 JSON，不要其他内容。`
    ...
    return `你是一个专业的元认知教练。在优先级层面，你的目标是通过具体场景化的问题，帮助用户**自然而然地发现**哪个任务更优先。
...
❌ **禁止直接问抽象概念**（像考试）：
- "你更关心紧急性还是重要性？"
- "「${taskNames[0] || '任务'}」是不是紧急又重要？"
- "哪个任务对目标影响更大？"

✅ **必须用具体场景引导**：

**1. 后果反推法**（自然识别紧迫性）
- "如果「${taskNames[0] || '任务'}」推迟到下周，会发生什么？"
- "「${taskNames[0] || '任务A'}」和「${taskNames[1] || '任务B'}」，哪个拖得越久越麻烦？"
- "有没有人在等你的「${taskNames[0] || '任务'}」结果？大概什么时候需要？"
...
**3. 行为观察法**（识别真实优先级）
- "「${taskNames[0] || '任务A'}」和「${taskNames[1] || '任务B'}」，哪个你一直在拖延？"
- "哪个任务你想到就焦虑，但又一直没开始？"
- "哪个任务做完后，你会觉得最轻松或最有成就感？"

**4. 依赖探索法**（识别顺序）
- "做完「${taskNames[0] || '任务'}」之后，会让其他什么事情变简单？"
- "这几个任务里，哪个是其他任务的前置条件？"

**问题选择要求**：
- 必须包含具体任务名称（${taskNames.join('、')}）
- 必须使用具体场景（"推迟会怎样""只能做一个""在拖延哪个"）
- 问题之间有逻辑递进
- 不要问子任务

【输出格式】
返回 JSON 数组（3-4 个问题，每个问题都是具体场景）：
[
  { "text": "问题内容（必须包含具体任务名称和场景）", "hint": "引导意图（如：后果反推、场景假设、行为观察）" }
]
...
`.trim()
    ...
    const systemPrompt = `你是一位擅长引导思考的智能助手。你的目标是：通过 2-4 个具体场景化的问题，帮助用户**自然而然地发现**哪个任务更优先，而不是直接问"哪个重要"。
...
✅ **要这样问**（具体场景，引导思考）：
- "如果「原型开发」推迟到明天，会影响到其他人或其他事吗？"
- "这周如果只完成一件事，你最希望是哪个？为什么？"
- "「原型开发」和「QE准备」，哪个你一直在拖延？通常拖延的原因是什么？"
...
**2. 场景假设法**（引导对比和权衡）
   - "这周如果只能完成一件事，你最希望是哪个？为什么？"
   - "如果明天有临时紧急事务，这几个任务里哪个可以先放一放？"
   - "假设你现在精力充沛，会从哪个任务开始？为什么？"

**3. 行为观察法**（识别真实优先级）
   - "「${taskNames[0] || '任务A'}」和「${taskNames[1] || '任务B'}」，哪个你一直在拖延？"
   - "哪个任务你想到就焦虑，但又一直没开始？"
   - "哪个任务做完后，你会觉得最有成就感或最轻松？"

**4. 依赖链探索**（识别执行顺序）
   - "做完「${taskNames[0] || '任务'}」之后，会让其他什么事情变简单？"
   - "这几个任务里，哪个是其他任务的前置条件？"
   - "如果「${taskNames[0] || '任务'}」卡住了，会影响到「${taskNames[1] || '任务'}」吗？"
...
- 具体、可视化的场景（"如果推迟""假设只能做一个"）
- 降低决策焦虑（不是"必须判断"，而是"想象一下"）
- 非评判性（关注事实和感受，不是"应该"）
...
用户回答后，AI 应该能够**推断出**（而不是用户直接说出）：
✓ 哪个任务更紧急（通过后果、依赖推断）
✓ 哪个任务更重要（通过长期价值、成就感推断）
✓ 任务的执行顺序（通过依赖关系推断）
✓ 用户的真实顾虑和障碍（通过拖延、焦虑推断）`

    const userPrompt = `请基于以下任务信息，生成 2-4 个引导性问题，帮助用户通过具体场景思考优先级：

${taskInfo}

请直接输出问题列表，每行以"- "开头。
**必须使用具体场景**（如"推迟会怎样""只能做一个""在拖延哪个"），而不是直接问"哪个重要"。
每个问题必须包含具体任务名称。`
    ...
    if (!response.success || !response.message) {
      ...
      if (taskNames.length >= 2) {
        return [
          `这周如果只能完成一件事，你最希望是哪个？为什么？`,
          `「${taskNames[0]}」和「${taskNames[1]}」，哪个拖得越久越麻烦？`,
          `有没有人在等你「${taskNames[0]}」或「${taskNames[1]}」的结果？`
        ]
      } else {
        return [
          `这周如果只能完成一件事，你最希望是哪个？为什么？`,
          `如果「${taskNames[0] || '这个任务'}」推迟到下周，会发生什么？`
        ]
      }
    }
    ...
    return questions.slice(0, 4) // 最多返回4个
