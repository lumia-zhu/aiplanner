...
export async function analyzeTasksForWorkflow(
  ...
  if (todayTasks.length === 0) {
    return {
      mode: 'single-task',
      reason: '你今天没有安排任何任务。可以先为今天添加一些任务,然后我再帮你完善计划。',
      confidence: 'low'
    }
  }
  ...
  if (todayTasks.length <= 2) {
    return {
      mode: 'single-task',
      reason: `你今天有 ${todayTasks.length} 个任务,数量较少。我建议逐个完善这些任务,明确细节和执行步骤,这样执行起来会更高效。`,
      confidence: 'high'
    }
  }

  // 分析任务描述情况
  const tasksWithSimpleDesc = todayTasks.filter(task => {
    const descLength = (task.description || '').trim().length
    return descLength < 20 // 描述少于20字符视为简单描述
  })

  const simpleDescRatio = tasksWithSimpleDesc.length / todayTasks.length

  // 情况3: 有很多描述简单的任务(超过50%)
  if (simpleDescRatio > 0.5) {
    return {
      mode: 'single-task',
      reason: `你今天有 ${todayTasks.length} 个任务,其中 ${tasksWithSimpleDesc.length} 个任务的描述比较简单。我建议先逐个完善这些任务,补充必要的细节、拆解复杂任务、估计时间,这样后续执行会更清晰。完善后我们再一起排列优先级。`,
      confidence: 'medium'
    }
  }

  // 情况4: 任务较多(>5个)且大部分描述完整
  if (todayTasks.length > 5 && simpleDescRatio < 0.3) {
    return {
      mode: 'priority-sort',
      reason: `你今天有 ${todayTasks.length} 个任务,而且大部分任务的信息都比较完整了。我建议直接使用优先级排序功能,帮你理清"先做什么、后做什么",让计划更有条理。`,
      confidence: 'high'
    }
  }

  // 情况5: 任务数量中等(3-5个)
  if (todayTasks.length >= 3 && todayTasks.length <= 5) {
    // 检查是否有标签信息
    const tasksWithTags = todayTasks.filter(task => (task.tags?.length ?? 0) > 0)
    const hasGoodTagging = tasksWithTags.length / todayTasks.length > 0.5

    if (hasGoodTagging) {
      return {
        mode: 'priority-sort',
        reason: `你今天有 ${todayTasks.length} 个任务,而且已经添加了不少标签信息。看起来你对任务的情况比较清楚,我建议直接排列优先级,制定执行计划。`,
        confidence: 'medium'
      }
    }
  }
  ...
  return {
    mode: 'single-task',
    reason: `你今天有 ${todayTasks.length} 个任务。我建议先逐个完善任务,明确每个任务的具体内容和执行步骤,然后再排列优先级。这样你会对整体计划更有把握。`,
    confidence: 'medium'
  }
}
 ...
 */
export function generateDetailedTaskSummary(tasks: Task[]): string {
  if (tasks.length === 0) {
    return '你今天没有安排任何任务。'
  }
  ...
  // 简洁的统计
  let summary = `你今天有 ${tasks.length} 个任务`
