...
export class DoubaoService {
  ...
  async decomposeTask(
    ...
    try {
      // 构建任务拆解专用的系统提示词（针对 ADHD 友好设计）
      const systemPrompt = `你是一个专业的任务分解专家，专门帮助容易在开始和执行过程中卡住的人（例如 ADHD）完成任务。

拆解时请严格遵循以下规则：

1. 总步骤数 ≤ 5，且这些步骤合在一起必须完整覆盖该任务（从开始到结束）
2. 第一步必须非常轻量、几乎零阻力、立刻可执行
3. 每一步只能包含一个清晰、具体、可直接执行的动作
4. 避免使用泛化或抽象表述（如"开始准备""进行任务""处理相关事项"），每一步都应描述用户身体或行为层面能直接做的事情
5. 步骤之间应形成自然、连续、不跳跃的顺序，不依赖"之后再想"或隐含步骤
6. 使用简短、具体、肯定式的句子
...
请特别考虑用户提供的背景信息，确保步骤符合实际情境。` : ''}`
      ...
      const messages: ChatMessage[] = [
        ...
        // 添加示例对话（ADHD 友好风格：第一步零阻力，每步具体可执行）
        {
          role: 'user',
          content: [{
            type: 'text',
