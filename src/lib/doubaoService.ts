...
export class DoubaoService {
  ...
  async decomposeTask(
    ...
    try {
      // 构建任务拆解专用的系统提示词（针对 ADHD 友好设计 + 禁止假设）
      const systemPrompt = `你是一个专业的任务分解专家，专门帮助容易在开始和执行过程中卡住的人（例如 ADHD）完成任务。

【核心原则】

1. **仅基于已知信息拆解**
   - 只能使用用户明确提到的信息和上下文
   - 禁止假设任务的具体场景、工具界面、详细流程
   - 当信息不够具体时，使用"通用框架"而非"具体想象"

2. **拆解规则**
   - 总步骤数 ≤ 5，且这些步骤合在一起必须完整覆盖该任务（从开始到结束）
   - 第一步必须非常轻量、几乎零阻力、立刻可执行
   - 每一步只能包含一个清晰、具体、可直接执行的动作
   - 步骤之间应形成自然、连续、不跳跃的顺序
   - 使用简短、具体、肯定式的句子

3. **通用 vs 具体的平衡**
   - 信息充足时：步骤可以具体（如"打开 PPT 软件，把3个要点复制进去"）
   - 信息不足时：步骤应通用（如"准备好需要用到的工具和材料"）
   - 使用"你要用的/你准备的"而非臆测具体内容
...
任务："介绍原型怎么用"
1. 打开你要演示的原型，定位到起始界面
2. 用一句话说清楚"这个原型是干什么用的"
3. 演示最核心的1-2个功能，边操作边讲解
4. 让对方跟着你操作一遍这个核心流程
5. 确认对方能独立重复一次操作

❌ 错误示范（假设细节）：
1. 拿出原型设备，打开首页的"任务录入"按钮 ← 假设了界面
2. 指着配图说"点击这个按钮能打开原型首页" ← 臆测内容

输出格式要求：
- 必须严格按照JSON格式返回，不要添加任何解释文字
- JSON格式必须严格正确，所有字符串必须用双引号包围

返回格式：
{
  "subtasks": [
    { "title": "具体的步骤描述", "order": 1 }
  ]
}`

      // 构建用户消息
      const userMessage = `请将以下任务拆解为最多5个步骤：
...
请特别考虑用户提供的背景信息，确保步骤符合实际情境。` : ''}`
      ...
      const messages: ChatMessage[] = [
        ...
        // 添加示例对话（展示信息充足和信息不足时的正确拆解方式）
        // 示例1：信息充足，可以具体拆解
        {
          role: 'user',
          content: [{
            type: 'text',
            text: '请将以下任务拆解为最多5个步骤：\n\n任务标题：准备学术会议演讲\n任务描述：需要在下周的学术会议上做20分钟的演讲'
          }]
        },
        {
          role: 'assistant',
          content: [{
            type: 'text',
            text: '{"subtasks":[{"title":"打开电脑，新建一个文档，写下演讲的核心主题一句话","order":1},{"title":"列出3个你最想讲的要点，每个要点写一行","order":2},{"title":"打开PPT软件，把3个要点复制进去作为3页标题","order":3},{"title":"对着镜子把3个要点各讲一遍，每个控制在5分钟内","order":4},{"title":"找一个朋友或家人，完整讲一遍并计时","order":5}]}'
          }]
        },
        // 示例2：信息较少，使用通用框架而非假设细节
        {
          role: 'user',
          content: [{
            type: 'text',
        ...
        },
        {
          ...
          content: [{
            type: 'text',
            text: '{"subtasks":[{"title":"打开你要演示的原型，定位到起始界面","order":1},{"title":"用一句话说清楚这个原型是干什么用的","order":2},{"title":"演示最核心的1-2个功能，边操作边讲解","order":3},{"title":"让对方跟着你操作一遍这个核心流程","order":4},{"title":"确认对方能独立重复一次操作","order":5}]}'
          }]
        },
        {
          role: 'user',
